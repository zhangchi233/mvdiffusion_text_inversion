...
class MP3Ddataset(torch.utils.data.Dataset):
    def __init__(self, config, mode='train'):
        ...
        if mode=='train':
            ...
            self.data = [data for data in self.data if "2t7WUuJeko7" in data[0]]
        else:
...
if __name__ =="__main__":
    ...
    root_dir = "G:\git_project\MVDiffusion\mp3d_skybox (1)\data"
    folders = os.listdir(root_dir)
    zip_dir =["matterport_color_images.zip","matterport_depth_images.zip","matterport_skybox_images.zip",
              'cameras.zip','matterport_camera_intrinsics.zip','matterport_camera_poses.zip',

            #  "matterport_normal_images.zip","matterport_semantic_images.zip",
              ]
    # loading the temp.zip and creating a zip object
    download_folder = os.listdir("G:\git_project\MVDiffusion\mp3d_skybox (1)\mp3d_skybox")


    for folder in folders:
        for zip_file in zip_dir:
            with ZipFile(os.path.join(root_dir,folder,"v1\scans",folder,zip_file), 'r') as zObject:
                # Extracting all the members of the zip
                # into a specific location.
                path = "G:\git_project\MVDiffusion\mp3d_skybox (1)\mp3d_skybox"
                print(os.path.join(root_dir,"data2",folder))
                zObject.extractall(
                    path=path)
