...
class ScannetDepthdataset(torch.utils.data.Dataset):
        ...
        for scene_id in sorted(scenes):
            ...
            for i, kf_id in enumerate(kf_ids):
                ...
                if os.path.exists(prompt_path):
                    ...
                    if mode == 'train' or i == 0 or (
                            config['data_load_mode'] == 'fix_frame' and i % config['num_views'] == 0):
    ...
    def load_seq(self, image_paths, resolution):
        ...
        depths = [cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX,cv2.CV_32F) for depth in depths]
        depths = [cv2.cvtColor(depth, cv2.COLOR_GRAY2BGR) for depth in depths]
        h_ori, w_ori,_ = depths[0].shape
        ...
        depth_min = [depth_inv[i][depth_valid_mask[i]].min()
                     for i in range(depth_inv.shape[0])]
        depth_max = np.stack(depth_max, axis=0)[:, None, None,None]
        depth_min = np.stack(depth_min, axis=0)[
                    :, None, None,None]  # [num_views, 1, 1]
    ...
    def __getitem__(self, idx):
        ...
        depth_inv_norm_small = np.stack([cv2.resize(depth_inv_norm[i], (
            self.resolution[0] // 8, self.resolution[1] // 8), interpolation=cv2.INTER_NEAREST) for i in
                                         range(depth_inv_norm.shape[0])])
        images_norm_small = np.stack([cv2.resize(images[i], (
            self.resolution[0] // 8, self.resolution[1] // 8), interpolation=cv2.INTER_NEAREST) for i in
                                      range(images.shape[0])])
        images_norm_small = rearrange(images_norm_small, 'm h w c -> m c h w')
        ...
        return {
            ...
            'images_norm_small': images_norm_small
        }