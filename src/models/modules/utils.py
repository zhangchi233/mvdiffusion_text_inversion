...
def transpose(R, t, X):
    b, h, w, c = X.shape # b,h,w,c,channel
    X = rearrange(X, 'b h w c-> b c (h w)')
    R = R.repeat(int(X.shape[0]/R.shape[0]), 1, 1)
    t = t.repeat(int(R.shape[0]/t.shape[0]), 1)
    X_after_R = R@X + t[:, :, None]

    X_after_R = rearrange(X_after_R, 'b c (h w) -> b h w c', h=h, w = w)
    return X_after_R
...
def get_x_2d(width, height): # need depth
    x = np.arange(width)
    y = np.arange(height)
    x, y = np.meshgrid(x, y)
    z = np.ones_like(x)
