...
class MultiViewBaseModel(nn.Module):
    def __init__(self, unet, config):
        ...
        self.condition_conv_in = nn.Conv2d(
            in_channels=7, out_channels=self.unet.conv_in.out_channels, kernel_size=3, stride=1, padding=1)
    ...
    def get_correspondence(self, cp_package):
        ...
        K = cp_package['K']
        depths = cp_package['images']
        b, m, h, w, c = depths.shape

        correspondence = torch.zeros(b, m, m, h, w, c, 2, device=depths.device)
        # m is m views, h, w is the height and width of the image
        ...
        for i in range(m):
            ...
            depth_i = depths[:, i:i+1].repeat(1, m, 1, 1,1)
            pose_j = poses
            depth_i = rearrange(depth_i, 'b m h w c -> (b m) h w c')
            ...
            point_ij = rearrange(point_ij, '(b m) h w c d -> b m h w c d', b=b)
            correspondence[:, i] = point_ij
            mask=(point_ij[:,:,:,:,:,0]>=0)&(point_ij[:,:,:,:,:,0]<w)&(point_ij[:,:,:,:,:,1]>=0)\
                 &(point_ij[:,:,:,:,:,1]<h)
            mask=rearrange(mask, 'b m h w c -> b m (h w c)')
...
class MultiViewDepthDeamer(MultiViewBaseModel):
    def __init__(self, unet, config):
        super().__init__(unet, config)
        self.camera_embedding = nn.Linear(3*4, 1280)