...
from ..modules.utils import get_x_2d,transpose,x_2d_coords
...
def back_projection(depth, pose, K, x_2d=None):
    ...
    X_world = X_world.reshape((-1, h, w, 3,3))
    return X_world

def get_correspondence(depth, pose, K, x_2d):
    b, h, w,c = depth.shape
    x3d = back_projection(depth, pose, K, x_2d)
    x3d = rearrange(x3d, 'b h w c d -> b c (h w d)')
    x3d = K[:, :3, :3]@x3d
    x3d = rearrange(x3d, 'b c (h w d) -> b h w d c', h=h, w=w)
...
def get_query_value(query, key_value, xy_l,
                    xy_r, depth_query, depths,
                    pose_rel, K, img_h_l, img_w_l, img_h_r=None, img_w_r=None):
    if img_h_r is None:
        img_h_r = img_h_l
        img_w_r = img_w_l
    ...
    for i in range(m):
        ...
        _key_value, _xy, _mask = get_key_value(key_value[:, i], xy_l[:, i], xy_r[:, i],
                                               depth_query, depths[:, i], pose_rel[:, i], K,
