...
class CPAttn(nn.Module):
    ...
    def forward(self, x, meta, img_h, img_w, R, K, m,depths,T):
        b, c, h, w = x.shape # the latent shape
        ...
        for i in range(m):
            ...
            xy_l=meta["correspondences"][:, i, indexs] # 2,h,w,2
            xy_r=["correspondences"][:, indexs, i]
           
            x_left = x[:, i] # b,c,h,w
            x_right = x[:, indexs] # b,2,c,h,w
            ...
            T_right = T[:, indexs]
            T_right = T_right.reshape(-1, 3, 1)
            T_left = T_left.reshape(-1, 3, 1)

            Depth_right = depths[:, indexs]
